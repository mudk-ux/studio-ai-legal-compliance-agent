...
Implements native `CoordinatorAgent`, `ScriptClearanceAgent`, `BrandExclusivityAgent`,
and `RemediationSlateAgent` with explicit model routing (gemini-2.5-flash vs gemini-3.5-pro)
and explicit Human-in-the-Loop execution pause hooks (`hitl_controller.py`).
"""
...
from src.schemas import IntakeConstraints
        ...
        with start_trace_span(
            ...
            if self.name == "ScriptClearanceAgent" and asset_type == "TEXT_SCREENPLAY":
                if asset_path.startswith("gs://"):
                    ...
                    bucket_name, blob_name = asset_path[5:].split("/", 1)
            ...
            # 2. BrandExclusivityAgent (Multimodal Temporal Video & Visual Image Perception)
                ...
                if asset_type == "VISUAL_IMAGE":
                    ...
                    for logo in logo_res.get("logos_detected", []):
                        ...
                        is_breach = any(
                            c.lower() in brand.lower() for c in excl.restricted_competitors
                        ) or any(
                            c.lower() in brand.lower() for c in ["coca-cola", "coca", "coke", "louis vuitton", "winston"]
                        )
                        findings.append({
                            "severity": "CRITICAL" if is_breach else "HIGH",
                            "category": "VISUAL_LOGO_EXCLUSIVITY_BREACH" if is_breach else "UNLICENSED_COMMERCIAL_LOGO",
                            "entity": brand,
            ...
            for f in findings:
                if f.get("severity") in ("CRITICAL", "HIGH"):
                    f_clean = {k: v for k, v in f.items() if k != "hitl_execution_pause"}
                    hitl_payload = pause_execution_for_human_review(
                        violation=f_clean, asset_reference=asset_path
                    )
